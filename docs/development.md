...
Скрипт устанавливает и настраивает PostgreSQL, создаёт роль `scale_user`, правила локального доступа и задание очистки. При каждом запуске он удаляет существующую `scale_db` вместе со всеми данными и создаёт её заново по `install/database/scale_db.sql`. Прямой запуск этого SQL-файла подходит только для подготовленного тестового окружения и не заменяет штатный установщик.

...
`MainForm` является навигационным хабом. При открытии формы взвешивания АЦП
оставляется открытым, а при открытии сервисных/корректировочных/печатных форм
соединение с АЦП закрывается.

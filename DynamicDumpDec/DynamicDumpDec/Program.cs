...
internal static class Program
{
    ...
    private static void Dump(string portName, int packetCount)
    {
        ...
        {
            blockIndex++;
            Console.WriteLine($"BLOCK={blockIndex:000}");
            ...
            for (var index = 0; index < packetCount; index++)
            {
                ...
                var timeMs = (int)Math.Round(blockSw.Elapsed.TotalMilliseconds);
                ...
                Console.WriteLine(
                    $"{totalIndex:000000} {blockIndex:000} {index:000} {timeMs:000000} {buffer[0]:000} {buffer[1]:000} {buffer[2]:000} {buffer[3]:000} {ch0:00000} {ch1:00000}");

                totalIndex++;
            }

            blockSw.Stop();
            var elapsedSec = Math.Max(blockSw.Elapsed.TotalSeconds, 0.001);
    ...
    private static void PrintHeader(string portName, int packetCount)
    {
        ...
        Console.WriteLine($"PACKETS_PER_BLOCK={packetCount}");
        Console.WriteLine($"PACKET_SIZE={PacketSize}");
        Console.WriteLine("FORMAT=CH0_UINT16_LE_CH1_UINT16_LE");
        Console.WriteLine("IDX BLOCK ROW TIME_MS B0 B1 B2 B3 CH0 CH1");
    }

...
Скрипт устанавливает и настраивает PostgreSQL, создаёт роль `scale_user` и правила локального доступа. При каждом запуске он удаляет существующую `scale_db` вместе со всеми данными и создаёт её заново по `install/database/scale_db.sql`. Прямой запуск этого SQL-файла подходит только для подготовленного тестового окружения и не заменяет штатный установщик.
...
Установщик не создаёт отдельного задания очистки: после установки приложение само
очищает результаты взвешивания и аудит по 60-дневному циклу. Подробности —
`docs/database.md`, раздел «Очистка локальных данных».
...
- Проверять установщик и принудительное пересоздание базы на чистой тестовой Windows-ВМ.

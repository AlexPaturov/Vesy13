...
internal static class Program
{
    ...
    private const string ModeName = "STATIC";
    private const byte SetByte = 86;
    private const byte ReqByte = 214;
    ...
    private static void Dump(string portName, int packetCount)
    {
        ...
        var hz = index / elapsedSec;

        Console.WriteLine("SUMMARY");
        Console.WriteLine($"BYTES={bytesTotal}");
        Console.WriteLine($"PACKETS={index}");
        ...
        Console.WriteLine($"CH0_AVG={(ch0Sum / (double)index).ToString("0.00", CultureInfo.InvariantCulture)}");
        Console.WriteLine($"CH0_MAX={ch0Max}");
        Console.WriteLine($"CH1_MIN={ch1Min}");
        Console.WriteLine($"CH1_AVG={(ch1Sum / (double)index).ToString("0.00", CultureInfo.InvariantCulture)}");
    ...
    private static void PrintHeader(string portName, int packetCount)
    {
        ...
        Console.WriteLine("REQUEST_MODE=POLL_PER_PACKET");
    ...
    {
        Console.Error.WriteLine("USAGE=StaticDumpDec COM3 60");
    }
}

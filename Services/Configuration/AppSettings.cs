...
public sealed class AppSettings
{
    ...
    /// <summary>
    /// Дата начала текущего 60-дневного периода очистки локальной БД.
    /// При первом запуске устанавливается текущая дата; после успешной очистки
    /// будет заменена датой её выполнения.
    /// </summary>
    public DateTime? DatabaseCleanupPeriodStartedOn { get; set; }

    /// <summary>
    /// Дата последней попытки очистки локальной БД. Нужна, чтобы после ошибки
    /// не выполнять несколько попыток в один календарный день.
    /// </summary>
    public DateTime? DatabaseCleanupLastAttemptedOn { get; set; }
